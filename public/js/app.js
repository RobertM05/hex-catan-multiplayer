...
class CatanApp {
  ...
  activateBuildCityWall() {
    ...
    const me = this.gameState.players.find(p => p.id === this.myPlayerId);
    const validIds = new Set();
    for (const vid of me?.citiesBuilt || []) {
      const building = this.gameState.grid.vertices[vid]?.building;
      if (building && building.playerId === this.myPlayerId && !building.hasWall) validIds.add(vid);
    }
  ...
  renderKnightSupply(me, isActionPhase) {
    ...
    if (wallBtn) {
      const canWall = isActionPhase && (me.cityWalls || 0) > 0 && (me.resources?.brick || 0) >= 2;
      wallBtn.disabled = !canWall;
      wallBtn.title = i18n.t('BUILD_CITY_WALL_COST');
    }
  }
  ...
  renderImprovementPanel(me, isActionPhase) {
    ...
    for (const [track, commodity] of Object.entries(tracks)) {
      ...
      const canAfford = hasCities && isActionPhase && level < 6 && have >= cost;
      ...
      if (levelEl) levelEl.textContent = `${level}/6`;
      if (fillEl) fillEl.style.width = `${(level / 6) * 100}%`;
      if (costEl) costEl.textContent = level >= 6 ? '—' : String(cost);
  ...
  getBestBankRatio(resource) {
    ...
    if (!me) return 4;
    if ((me.cityImprovements?.trade || 0) >= 1) return 2;
    let bestRatio = 4;
  ...
  setupTradeModals() {
    ...
    document.getElementById('btn-open-trade').addEventListener('click', () => {
      resetTradeSteppers();
      this.renderBankTradeUI();
      const isMyTurn = this.gameState && this.gameState.players[this.gameState.currentTurnPlayerIndex]?.id === this.myPlayerId;
  ...
  renderGameState() {
    ...
    if (s.phase === 'SETUP_ROUND_1' || s.phase === 'SETUP_ROUND_2') {
      ...
      if (chooserId === this.myPlayerId) {
        const me = s.players.find(p => p.id === this.myPlayerId);
        const validIds = new Set((me?.citiesBuilt || []).filter((vid) => {
          const b = s.grid.vertices[vid]?.building;
          return b && b.playerId === this.myPlayerId && !b.hasMetropolis;
        }));
        this.selectedAction = { type: 'metropolis', validIds };
        this.boardRenderer.render(s.grid, this.selectedAction, null, s.players);
      } else {
        this.boardRenderer.render(s.grid, null, null, s.players);
      }
    ...
    if (tradeModal && tradeModal.classList.contains('active')) {
      const isMyTurn = s.players[s.currentTurnPlayerIndex]?.id === this.myPlayerId;
