...
describe('UX-04 phase to status mapping', () => {
  it('maps each known phase to the expected key for the active player', () => {
    ...
    assert.equal(mapPhaseToStatusKey('TURN_CHOOSE_METROPOLIS', true), 'STATUS_YOUR_METROPOLIS');
    assert.equal(mapPhaseToStatusKey('TURN_CHOOSE_KNIGHT_RELOCATE', true), 'STATUS_YOUR_KNIGHT_RELOCATE');
